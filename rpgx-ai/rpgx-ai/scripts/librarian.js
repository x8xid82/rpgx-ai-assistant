...
class RPGXKnowledgeBase extends FormApplication {
  ...
  async _build() {
    ...
    const actionsHtml = `
      <div class="rpgxkb-actions">
        ...
        <button type="button" class="rpgxkb-btn rpgxkb-btn-neutral kb-ingest-pdf" title="Ingest a PDF file (text or scanned)">
          <i class="fa-solid fa-file-pdf"></i> Ingest PDF
        </button>
        <button type="button" class="rpgxkb-btn rpgxkb-btn-neutral kb-ingest-image" title="Ingest an image — AI describes and indexes it">
          <i class="fa-solid fa-image"></i> Ingest Image
        </button>
    ...
    const html = `
      <div class="rpgxkb-root">
        ...
        <input type="file" id="rpgxkb-image-input" accept=".jpg,.jpeg,.png,.webp,.gif"
               style="display:none;position:absolute;" />
      </div>`;
  ...
  _bindEvents(el, allDocs) {
    ...
    // Image ingest — trigger hidden file input
    el.find(".kb-ingest-image").on("click", () => {
      const input = document.getElementById("rpgxkb-image-input");
      if (input) { input.value = ''; input.click(); }
    });

    // File input change handlers — addEventListener for CSP compliance (no inline onclick)
    ...
    const imgInput = document.getElementById("rpgxkb-image-input");
    if (imgInput) {
      imgInput.addEventListener("change", (e) => {
        const file = e.target.files?.[0];
        if (file) this._ingestImage(file);
      });
    }
  ...
  // ── Core ingest sender — batches 3 docs at a time with 300ms gaps ─────────────
  // onProgress({ batchNum, totalBatches, processed, total, docNames }) is called
  // before each batch so the caller can update the progress bar.
  async _ingestDocs(docs, onProgress) {
    const ragBase      = getSetting("ragBase");
    const worldId      = game.world?.id;
    const chunkSize    = 400;  // hardcoded — the paragraph-first chunker has its own rules;
    const chunkOverlap = 0;    // user-facing controls were removed to prevent overrides.
    const BATCH_SIZE   = 3;
  ...
  // ── Ingest PDF ────────────────────────────────────────────────────────────────
  // Reads a PDF file, sends it to /ingest/pdf for text extraction.
  // If the server reports a scanned PDF (no text layer), falls back to
  // _ingestPDFAsImages() which renders each page as a canvas and sends
  // each page image to /ingest/image for moondream description.
  async _ingestPDF(file) {
    ...
    try {
      ...
      if (j.isScanned) {
        // Scanned PDF — fall back to page-by-page image ingestion via PDF.js
        this._logProgress(`⚠ Scanned PDF (${j.pages} pages) — switching to image ingestion…`);
        await this._ingestPDFAsImages(file, arrayBuffer, docId, docTitle);
        return;
      }
  ...
  // ── Ingest Scanned PDF as Images ──────────────────────────────────────────────
  // Called when /ingest/pdf returns isScanned:true (no text layer detected).
  // Uses PDF.js (available in Foundry v12+) to render each page as a canvas,
  // then sends each page as a base64 image to /ingest/image for moondream.
  // Note: accuracy is lower than text extraction — moondream describes content
  // rather than transcribing verbatim. Good for campaign content; lossy for stat tables.
  async _ingestPDFAsImages(file, arrayBuffer, docId, docTitle) {
    const ragBase = getSetting("ragBase");
    const worldId = game.world?.id;

    // PDF.js is bundled with Foundry v12+ and available as a global
    const pdfjs = globalThis.pdfjsLib ?? null;
    if (!pdfjs) {
      this._logProgress('✗ PDF.js not available. Scanned PDF image ingestion requires Foundry v12+.');
      ui.notifications.error('Scanned PDF ingestion requires Foundry v12+.');
    ...
    try {
      ...
      this._logProgress(`Rendering ${totalPages} pages for AI description…`);

      let added = 0;
      for (let i = 1; i <= totalPages; i++) {
        this._setProgress(Math.round((i / totalPages) * 90) + 5, `Page ${i} / ${totalPages}`);
        this._logProgress(`  → Page ${i}: rendering…`);
        ...
        const pageDocId  = `${docId}:page${i}`;
        const pageTitle  = `${docTitle} — Page ${i}`;
        const context    = `page ${i} of ${totalPages} from PDF "${docTitle}"`;

        const res = await fetch(`${ragBase}/ingest/image`, {
          method:  'POST',
          headers: { 'Content-Type': 'application/json', ...ragAuthHeader() },
          body:    JSON.stringify({
            worldId, docId: pageDocId, docTitle: pageTitle,
            docType: 'pdf', base64, context,
          }),
        });

        if (res.ok) {
          const j = await res.json();
          if (j.added) { added++; this._logProgress(`  ✓ Page ${i} described and stored`); }
          else          { this._logProgress(`  ⚠ Page ${i} — AI returned no description`); }
        }
      }

      this._setProgress(100, 'Complete!');
      this._logProgress(`✓ ${added} / ${totalPages} pages ingested from ${docTitle}`);
      ui.notifications.info(`Scanned PDF ingested: ${added} pages.`);

    } catch (e) {
      this._logProgress(`✗ Image rendering error: ${e.message}`);
      ui.notifications.error(`Scanned PDF ingest failed: ${e.message}`);
    }
  ...
  // ── Ingest Image ──────────────────────────────────────────────────────────────
  // Reads an image file, sends it to /ingest/image for moondream description.
  // Supported: jpg, jpeg, png, webp, gif
  // The AI description is stored as a single searchable chunk in the world DB.
  async _ingestImage(file) {
    if (!file) return;

    if (file.size > 15 * 1024 * 1024) {
      ui.notifications.error(`Image is too large (${(file.size/1024/1024).toFixed(1)} MB). Maximum is 15 MB.`);
      return;
    }
    ...
    const worldId  = game.world?.id;
    const safeName = file.name.replace(/[^a-zA-Z0-9_\-]/g, '_');
    const docId    = `img:${safeName}`;
    const docTitle = file.name.replace(/\.[^.]+$/, '');

    this._showProgress(true);
    this._setProgress(15, `Reading ${file.name}…`);
    this._logProgress(`🖼️ Image: ${file.name}`);

    try {
      const arrayBuffer = await file.arrayBuffer();
      const base64      = arrayBufferToBase64(arrayBuffer);

      this._setProgress(40, 'Sending to AI for description…');
      this._logProgress('Describing with moondream (this may take a moment)…');

      const res = await fetch(`${ragBase}/ingest/image`, {
        method:  'POST',
        headers: { 'Content-Type': 'application/json', ...ragAuthHeader() },
        body:    JSON.stringify({ worldId, docId, docTitle, docType: 'image', base64 }),
      });
      ...
      if (j.skipped) {
        this._logProgress(`⚠ ${docTitle} — AI returned no description`);
        ui.notifications.warn('Image processed but AI returned no description. Try a different image.');
      } else {
        this._logProgress(`✓ ${docTitle} — described and stored as 1 chunk`);
        ui.notifications.info(`Image ingested: "${docTitle}".`);
      }

    } catch (e) {
      this._logProgress(`✗ Error: ${e.message}`);
      ui.notifications.error(`Image ingest failed: ${e.message}`);
    }
